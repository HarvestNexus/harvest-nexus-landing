      ...
      >
        ...
        <div className="space-y-8">
          <div>
            ...
            <p className="mt-4 text-gray-600">{/* optional sub-text */}</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6">
            {features.map((feature, index) => (
              <div
                key={index}
                className="flex flex-col rounded-xl items-start gap-2 px-2 py-2 
                           transition-transform duration-300 ease-in-out 
                           hover:scale-105 hover:shadow-lg hover:bg-secondary"
              >
                {/* Wrap icon in Suspense for lazy loading */}
                <Suspense fallback={<div className="w-7 h-7 bg-gray-200 rounded-full" />}>
